	...
	for (let attempt = 1; attempt <= maxAttempts; attempt++) {
		try {
			...
			if (attempt < maxAttempts) {
				// Don't show retry messages - they're redundant and annoying
				// The offline banner already indicates connection issues
				...
				// Only show error if we're online - offline banner already communicates the issue
