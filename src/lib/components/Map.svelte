<script lang="ts">
	...
	interface Props {
		...
		topPadding?: number;
		leftPadding?: number;
		flyToCoords?: (lat: number, lng: number) => void;
	}
	...
	let {
		...
		flyToCoords = $bindable((lat: number, lng: number) => {
			if (map && mapLoaded) {
				map.flyTo({
					center: [lng, lat],
					zoom: 16,
					padding: {
						top: topPadding,
						bottom: Math.min(bottomPadding, window.innerHeight / 2),
						left: leftPadding,
					},
				});
			}
	...
	currentPos.subscribe((pos: Position | null) => {
		...
		if (pos && pos.coords) {
			...
			const src =
				map.getSource<maplibregl.GeoJSONSource>("user-location");
			const data: GeoJSON = {
	...
	// Refresh map when highlighted stations change (for destination finder pulsing)
	highlightedStations.subscribe(() => {
		if (!mapLoaded) return;
		setSourceData(map);
	});
...
{#if !ready}
	<div
		...
		class="blur fixed bg-cover top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[2000px] h-[2000px] z-10 bg-[url(/assets/map-preview-light.jpg)] dark:bg-[url(/assets/map-preview-dark.jpg)]"
	></div>
	<svg
		out:fade={{ duration: 500 }}
		class="absolute w-20 h-12 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 transition-opacity {$token ===
		null
			? 'opacity-0'
			: 'opacity-100'}"
		width="62"
